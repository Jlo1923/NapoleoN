...
import com.naposystems.napoleonchat.source.remote.dto.conversation.deleteMessages.DeleteMessage422DTO
...
class ConversationRepository @Inject constructor(
    ...
    override suspend fun setMessageRead(messageWebId: String) {
        try {
            ...
            if (messageAndAttachment?.messageEntity?.status == Constants.MessageStatus.UNREAD.status &&
                messageAndAttachment.messageEntity.isMine == Constants.IsMine.NO.value
            ) {
                val response = napoleonApi.sendMessagesRead(
                    MessagesReadReqDTO(
