package com.naposystems.napoleonchat.utility.extras

const val MULTI_EXTRA_CONTACT = "contact"
const val MULTI_EXTRA_FILES = "files"
const val MULTI_EXTRA_FILES_DELETE = "files_delete"
const val MULTI_SELECTED = "multi_selected"
const val MODE_ONLY_VIEW = "mode_only_view"
const val MESSAGE_TEXT = "message_text"

const val EXTRA_MULTI_MSG_TO_SEND = "EXTRA_MULTI_MSG_TO_SEND"
const val EXTRA_MULTI_ATTACHMENTS_TO_SEND = "EXTRA_MULTI_ATTACHMENTS_TO_SEND"

const val IDS_TO_DELETE = "IDS_TO_DELETE"

const val MULTI_ATTACHMENT_PREVIEW_INTENT = 1000
const val MULTI_ATTACHMENT_INTENT = 1001